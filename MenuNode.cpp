...
void print_xml(const char* message, const pugi::xml_document& doc, const pugi::xml_parse_result& result)
{
    std::cout
    << message
    << "\t: load result '" << result.description() << "'"
    << ", first character of root name: " << pugi::as_wide(doc.first_child().name())[0]
    << ", value: " << doc.first_child().first_child().first_child().child_value()
    << std::endl;
}

ifstream & operator>>(ifstream &input, MenuNode &node){
    //next: import XML file and create nodes
    pugi::xml_document data;
    pugi::xml_parse_result result = data.load(input);
    
    print_xml("UTF8 file from narrow stream", data, result);
    
    cout << "Provider: " << node.t << endl;
    
    pugi::xml_node items = data.child("rss").child("channel");
    for (pugi::xml_node item = items.child("item"); item; item = item.next_sibling("item"))
    {
        //cout << "Item: ";
        Item * newItem = new Item(node.t);
        ...
        //add item
            //A) drop item if:
                //1. category is in ignore list
                //2. price attribute is not empty
                //3. ignore empty sources
